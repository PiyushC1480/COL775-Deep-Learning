...
class ARGS():
    def __init__(self):
        self.model_type = "lstm_lstm_attn"
...
#         print(src.shape)
        ...
        return outputs , hidden.unsqueeze(0), cell.unsqueeze(0) #hidden = [num_layers, batch_size, hidden_dim] , cell = [num_layers, batch_size, hidden_dim]
...
class AttentionNetwork(nn.Module):
    def __init__(self, hidden_dim):
        ...
        self.attn = nn.Linear((hidden_dim*4),2*hidden_dim)
        self.v = nn.Linear(2*hidden_dim, 1, bias = False)
        
    def forward(self, hidden, encoder_outputs):
        #hidden = [num_layers, batch_size, hidden_dim], encoder_outputs = [batch_size, src_len, hidden_dim]
        batch_size = encoder_outputs.shape[0]
        src_len = encoder_outputs.shape[1]
        
        hidden = hidden[-1].unsqueeze(1).repeat(1, src_len, 1) #hidden = [batch_size, src_len, hidden_dim]
#         print("Hidden :", hidden.shape)
        energy = torch.tanh(self.attn(torch.cat((hidden, encoder_outputs), dim = 2))) #energy = [batch_size, src_len, hidden_dim]
        attention = self.v(energy).squeeze(2) #attention = [batch_size, src_len]
        return F.softmax(attention, dim=1).unsqueeze(1) #return attention = [batch_size, 1, src_len]
#LSTM Decoder
class AttenDecoder(nn.Module):
    def __init__(self, output_dim, embed_dim, hidden_dim, num_layers, dropout, embed_matrix, attention):
        super().__init__()
        ...
        self.embedding.weight.data.copy_(embed_matrix)
        self.lstm = nn.LSTM(embed_dim + 2*hidden_dim, 2*hidden_dim, num_layers, dropout=dropout, batch_first=True)
        self.fc_out = nn.Linear(2*hidden_dim, output_dim)
        self.dropout = nn.Dropout(dropout)
        self.attention = attention
        
    def forward(self, input, hidden_cell, encoder_outputs):
        #input = [batch_size], hidden = [num_layers, batch_size, hidden_dim], cell = [num_layers, batch_size, hidden_dim], encoder_outputs = [batch_size, src_len, hidden_dim]
        input = input.unsqueeze(1) #input = [batch_size, 1]
        embedded = self.dropout(self.embedding(input)) #embedded = [batch_size, 1, embed_dim]
        a = self.attention(hidden_cell[0], encoder_outputs) #a = [batch_size, 1, src_len]
        weighted = torch.bmm(a, encoder_outputs) #weighted = [batch_size, 1, hidden_dim]
        rnn_input = torch.cat((embedded, weighted), dim = 2) #rnn_input = [batch_size, 1, embed_dim + hidden_dim]
        output, (hidden, cell) = self.lstm(rnn_input, hidden_cell) #output = [batch_size, 1, hidden_dim], hidden = [num_layers, batch_size, hidden_dim], cell = [num_layers, batch_size, hidden_dim]
...
class Seq2Seq(nn.Module):
    ...
    def get_decoder(self):
        ...
        attention = AttentionNetwork(self.decoder_hidden_dim)
        return AttenDecoder(self.decoder_input_size, self.embed_dim, self.decoder_hidden_dim, self.num_layers, self.dropout, embed_matrix, attention)

    def forward(self, src, trg, teacher_forcing_ratio=0.6):
        #src = [batch_size, src_len], trg = [batch_size, trg_len]
        batch_size = trg.shape[0]
        trg_len = trg.shape[1]
        trg_vocab_size = self.decoder_input_size
        
        outputs = torch.zeros(batch_size, trg_len, trg_vocab_size).to(device) #outputs = [batch_size, trg_len, trg_vocab_size]
        encoder_outputs, hidden, cell = self.encoder(src) #encoder_outputs = [batch_size, src_len, hidden_dim], hidden = [num_layers, batch_size, hidden_dim], cell = [num_layers, batch_size, hidden_dim]
        input = trg[:, 0] #input = [batch_size]
...
#             if(i==4) :  break 
    ...
    print("Training and Dev Complete for Seq2Seq Atten model")
...
#     args.data_dir = os.path.join(pth, "data")
    ...
    test_dataset = TextToMathDataset(args.processed_data, test_to_load)
...
def run_test_S2SAtten(model_path , test_data_file):
    cwd  = os.getcwd()
